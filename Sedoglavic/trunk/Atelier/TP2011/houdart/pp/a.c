...
/* et oui pour voir */unsigned long long nb ;unsigned int nbproc ;pthread_t *threads ;struct data_m 
    ...
    /* indicates that a cancellation request is acted immediately */	pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS,NULL) ;	for(i=((data_t*) data)->start;	     i<((data_t *) data)->end; i++)		if (!(nb % i))		
