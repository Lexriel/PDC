...
void *searchdivisor(void * data)
{
    ...
    for(i=((data_t*) data)->start; i<((data_t *) data)->end; i++) if(!(nb % i))
    {
        printf(" %llu est un diviseur de %llu \n",i,nb);
        for(j=0;j<nbproc;j++) if(j!=((data_t *) data)->current_thread_nb) if(!pthread_cancel(threads[j])) printf("Incorrect cancellation");
        return NULL;
    }
    return NULL;
}
int main(void)
{
    ...
    for(i=0;i<nbproc;i++) pthread_join(threads[i],NULL);
    return 0;
}
