...
int main(void)
{
    unsigned long long sqrt_nb ;	unsigned long long tmp ;        int result ;	int i ;		data_t *data ;				nb = 4611686138686472687ULL ;	nbproc = sysconf(_SC_NPROCESSORS_ONLN) ;	printf("%d computing unit available\n",nbproc) ;	threads = (pthread_t *) malloc(nbproc*sizeof(pthread_t)) ;	data = (data_t *) malloc(nbproc*sizeof(data_t)); 	sqrt_nb = sqrt(nb) ; 	tmp = sqrt_nb 
    / nbproc ;	for(i=0;i<nbproc;i++) 	 {		data[i].start = 2+i*tmp ;		data[i].end = data[i].start + tmp ;		data[i].current_thread_nb = i ;		pthread_create(&threads[i],NULL,searchdivisor,				(void *) &data[i]) ;	 }			for(i=0;i<nbproc;i++)		pthread_join(threads[i],NULL) ;        return 0 ;	}*/
    /*