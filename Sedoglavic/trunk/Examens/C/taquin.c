...
void
initfile(struct file_t * file)
{
	file->tail = file->head = NULL;
}

int
estvide(struct file_t * file)
{
	return !(file->tail || file->head);
}

struct cellule_t *
creercellule(struct position_t * pos)
{
...
void
ajouter(struct file_t * file, struct cellule_t * cell)
{
...
char           *
deplacementpossible(int casevide[])
{
...
struct position_t
nouvelleposition(struct position_t res, char depl)
{
...
int
sontegales(struct position_t * pos1, struct position_t * pos2)
{
...
char           *
solve(struct position_t taquin)
{
